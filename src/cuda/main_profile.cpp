...
int main()
{
  ...
  int nSpherocyls = 16384;
  cout << "Number of particles = " << nSpherocyls << endl;
  double dPacking = 0.89;
  cout << "Packing fraction =  " << dPacking << endl;
  double dL = sqrt(0.5 * nSpherocyls * D_PI * (0.5 * 0.5 + 0.7 * 0.7) / dPacking);
  ...
  double *dR = new double[nSpherocyls];
  srand(time(0));
  ...
  Spherocyl_Box cTest(nSpherocyls, dL, dX, dY, dR, dEpsilon);
