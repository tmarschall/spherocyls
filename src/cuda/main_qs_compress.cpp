...
int main(int argc, char* argv[])
{
  ...
  double dResizeStep = float_input(argc, argv, ++argn, "Quasistatic resizing step size");
  cout << dResizeStep << endl;
  double dMinStep = float_input(argc, argv, ++argn, "Minimum relaxation step size");
  cout << dMinStep << endl;
  double dMaxE = float_input(argc, argv, ++argn, "Maximum energy during compress");
  cout << dMaxE << endl;
  int nCycles = int_input(argc, argv, ++argn, "Number of compression cycles");
  cout << nCycles << endl;
  ...
  cSpherocyls->quasistatic_find_jam(dMaxE, 1e-16, dResizeStep, dMinStep);
  cSpherocyls->calculate_stress_energy();
  cSpherocyls->display(1,0,0,1);
