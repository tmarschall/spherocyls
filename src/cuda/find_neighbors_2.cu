...
__global__ void get_nbr_blocks(int nSpherocyls, int *pnNNbrs, int *pnNbrList, int *pnBlockNbrs, int *pnBlockList)
{
  extern __shared__ int sData[];
  int thid = threadIdx.x;
  int blid = blockIdx.x;
  ...
  while (nPID < nSpherocyls) {
    ...
    int nID = sData[thid];
    for (int n = 0; n < nNbrs; n++) {
	sData[blsz + nID + n] = pnNbrList[n * nSpherocyls + nPID];
    }
    for (int t = nTotNbrs + thid; t < nSortMax; t += blsz) {
      sData[blsz + t] = nSpherocyls;
    }
	...
	dStep /= 2;
      ...
      pnNbrList[nSpherocyls*n + nPID] = newID;
    }
...
void Spherocyl_Box::find_neighbors()
{
  ...
  get_nbr_blocks <<<m_nGridSize, m_nBlockSize, m_nSM_GetNbrBlks>>>
    (m_nSpherocyls, d_pnNPP, d_pnNbrList, d_pnBlockNNbrs, d_pnBlockList);
  cudaThreadSynchronize();
